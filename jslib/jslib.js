class Animator {
  ...
  fadeIn(time, toggle = false) {
    if (toggle && this.selector.classList.contains('fadeIn-active')) {
      this.selector.style.opacity = 0;
      this.selector.style.transition = `all ${time}s ease`;
      this.selector.classList.remove('fadeIn-active');
    } else {
      this.selector.style.opacity = 1;
      this.selector.classList.add('fadeIn-active');
    }
  }
}
